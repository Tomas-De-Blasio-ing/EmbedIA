...
void svm_classifier_layer(const svm_classifier_layer_t *model, const data1d_t *input, data1d_t *output) {
    #define DFIX_ZERO FIXED_TO_DFIXED(FIX_ZERO)
    ...
    for(uint16_t i = 0; i < model->n_classes; i++) {
        ...
        for(uint16_t j = i + 1; j < model->n_classes; j++) {
            ...
            dfixed decision = dot_product_bias(K, model->ovo_coefs+pair_idx * model->n_SV, end_i, model->ovo_icepts[pair_idx]);
            decision += dot_product(K, model->ovo_coefs+pair_idx * model->n_SV, end_j);

            probabilities[decision > DFIX_ZERO ? i : j] += normalizer;
            pair_idx++;
        }
    }
}
...
void svm_linear_classifier_layer(const svm_linear_classifier_layer_t *model, const data1d_t *input, data1d_t *output)
{
    ...
    for(uint16_t class_idx = 0; class_idx < model->n_classes; class_idx++) {
        ...
        dfixed score = dot_product_bias(coef_ptr, input->data, model->n_features, model->ovr_icepts[class_idx]);
        if (score > DFIX_MAX)
            output->data[class_idx] = FIX_MAX;
        else if (score < DFIX_MIN)
            output->data[class_idx] = FIX_MIN;
        else
            output->data[class_idx] = DFIXED_TO_FIXED(score);
    }
}
