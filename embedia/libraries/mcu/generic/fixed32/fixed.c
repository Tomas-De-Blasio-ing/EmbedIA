...
fixed fixed_tanh(fixed x) {
    ...
    if(abs_x > FIX_TWO)
        return x > 0 ? FL2FX(0.999) : FL2FX(-0.999);
...
fixed fixed_tanh_precise(fixed x) {
    // Tabla expandida de 17 valores para mejor precisión (68 bytes)
    static const fixed tanh_table[17] = {
        FL2FX(0.0),       FL2FX(0.12435300), FL2FX(0.24491866), FL2FX(0.35994896),
        FL2FX(0.46211716), FL2FX(0.54930614), FL2FX(0.62245933), FL2FX(0.68350264),
        FL2FX(0.73462783), FL2FX(0.77830965), FL2FX(0.81723010), FL2FX(0.84828364),
        FL2FX(0.87344976), FL2FX(0.89411710), FL2FX(0.91173951), FL2FX(0.92705295),
        FL2FX(0.93969262)
    };
    ...
    // Saturación para valores grandes
    if (abs_x >= FL2FX(4.0)) {
        return x > 0 ? FL2FX(0.999) : FL2FX(-0.999); // Evita saturación exacta
    }

    // Mapeo: x en [0,4] -> índice en [0,16]
    fixed position = FIXED_MUL(abs_x, FL2FX(4.0));
    unsigned int idx = FIXED_TO_INT(position);

    // Protección contra desbordamiento de índice
    if (idx >= 16) idx = 15;
